    ...
    try:
        ...
        if os.path.isfile(csv_file_path):
            ...
            with open(csv_file_path, "a", newline="",encoding='utf-8') as csv_file:
                ...
                writer.writerow([filename,first_name, last_name, email])
        else:
            with open(csv_file_path, 'w', newline='',encoding='utf-8') as csv_file:
                writer = csv.writer(csv_file)
                writer.writerow(['Filename','First Name', 'Last Name', 'Email'])
                writer.writerow([filename,first_name, last_name, email])
    except Exception as e:
        print(f"{filename} is not readable...skipping...}")
        
    
if __name__ == '__main__':
    folder_path = './'
    pdf_files = [os.path.join(folder_path, file) for file in os.listdir(folder_path) if file.endswith('.pdf')]
    pool = multiprocessing.Pool(processes=10)  # Change the number of processes as per your requirement
    result = pool.map(read_pdf, pdf_files)
    pool.close()
    pool.join()
